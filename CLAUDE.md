...
shared domain core (`@sii/core`) backs two surfaces: a human **CLI** and an
...
- One package: `pnpm --filter @sii/core <script>`

## Critical Rules

- **Never hard-code a SII hostname.** Read from `@sii/core` config. Prod is the
  only target; there is no env switch (ported from sii-py ADR-016). (ADR-004)
- **Surfaces call `@sii/core` tasks only.** The CLI and MCP NEVER reach past the
