...
| 💭 | `sii f22 observaciones <año>` (#26) | `f22_observaciones` | **Inconsistencias / observaciones** — the discrepancies the estado glosa references ("diferencias entre tu info y la de los agentes retenedores/informantes"): the observación códigos (A##, …) + glosas + impugnación detail the taxpayer must resolve. New SDI facade → spike to locate the endpoint + a first-hand contract (ADR-004). Session-keyed; PII-safe. |
