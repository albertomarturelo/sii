...
| 📋 | `sii carpeta regular` (#109) / `sii carpeta instituciones` (#110) | `carpeta_regular` / `carpeta_instituciones` | **Carpeta Tributaria Regular as a local PDF** (discussion #87). Wire flow observed first-hand by a contributor 2026-09-10 (fork `golazoia/sii`): SPA-JSON on `www2.sii.cl/app/cte-api-carpetatributaria/{rut}/…` — a session warm-up `GET /app/session/status?originalUrl=…` (else 401), `POST …/generar` (`enfinCodigo` REQUIRED, from the LIVE `…/instituciones` list — hardcoded codes go stale) → `{carpCodigoRepositorio}`, then `GET …/pdfInicial/{codigo}` → `{base64}` (~500 KB, 10–20 s). `…/obtenerValorParametro` is NOT the PDF (a `text/plain` widget URL, #111). Descriptor not bytes (ADR-022); the Carpeta is the most PII-dense SII document. Auth mode UNCONFIRMED: the API path carries `{rut}`, so it may be body-RUT rather than session-keyed — probe live before wiring (ADR-005). Milestone 0.9.1. |
...
| Carpeta tributaria | `carpeta` | — | regular (PDF, #109), instituciones (#110) | SPA-JSON `www2` cte-api (`carpeta.ts`) · session-keyed expected, body-RUT possible (path carries the RUT) — confirm live |
