...
| ✅ | `sii bte list <periodo>` (#20) | `bte_list` | **Built + live-validated (#20, 2026-06-30).** Monthly boletas de honorarios for a período — `--recibidas` / `--emitidas` (default). **First inline-JS-map facade:** reads the legacy `loa.sii.cl/cgi_IMT/` CGIs' `xml_values` + `arr_informe_mensual` via `PortalSession.goto`/`evaluate` (NOT `requestJson`), paginated + paced. Curated boleta + `raw` (the per-boleta row); the taxpayer's OWN identity (in the report meta) is dropped. **Session-keyed** (ADR-005, spike #15 answered #62 + re-confirmed TS-live: `rut_arrastre` rides the principal; no `--rut`, rejects a representing pointer up front). es-CL monto parsing. Live-validated emitidas (cookie SSO-carry + evaluate + shape); recibidas rows ported (test account had none). Wire contract `sii-contract/bte.md`. |
