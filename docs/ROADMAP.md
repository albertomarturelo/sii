...
| Carpeta tributaria | `carpeta` | instituciones (built; live-validation pending #116) | regular (PDF, #109) — both need the www2 app session (`sii auth login --www2`, ADR-026 #116) | SPA-JSON `www2` cte-api (`carpeta-tributaria-regular.ts`) · session-keyed (the www2 app session is the principal; path keyed by its `userId`) |
