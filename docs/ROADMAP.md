...
| 🚧 | `sii f29 draft <periodo>` / `status <periodo>` | `f29_draft` / `f29_status` | **Built + tested (#18)** — session-keyed: `getDeclaracionConCondicionesYTipoPropuesta` (IVA propuesta → curated `codigos` + `codigosAdministrativos`) + `getDeclaracionConEstados` (presented-F29 records → estado/folio/fecha). Wire contract **ported** from the Python `sii-cli` (cited; `docs/sii-contract/f29.md`), **NOT yet live-revalidated from TS**. PII-safe: curated tax códigos only, NO `raw`; `listCodBase`/`traza`/`monto` dropped. **Session-keyed** (no `--rut`): rejects a representing operate pointer **up front** with an actionable "log in as the empresa" message (answers spike #15 for F29 via the port). |
...
- **Tools** (actions): ✅ `auth_login` (no password — delegates to the browser
  ...
  form, #37) + `f22_observaciones` + `f29_draft` / `f29_status` (`periodo`,
  session-keyed, #18), all `readOnlyHint`. Each is a
