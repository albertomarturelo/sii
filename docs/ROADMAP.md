...
| Status | CLI | MCP | Spec |
|---|---|---|---|
| 🚧 | `sii auth login` (browser) / `--console` / `--keyring` | `auth_login` (no password arg) | **CLI real-SII validated (#5)**; **MCP tool built + tested** (no password arg; delegates to the browser flow). **`--console`** (ADR-010): RUT + hidden Clave in the terminal → headless form-fill → same cookies-only session, Clave never stored. CLI-only. Headed login persists `~/.sii/session.json` (0600, no secret). **`--keyring`** (ADR-025): the Clave comes from the OS keyring (service `sii`, username = the RUT, tried canonical → dotted → body-only) and everything else matches `--console` — ONE attempt, cookies-only session, NO implicit re-login. CLI-only (the `/cli` subpath), never over MCP. |
...
| 🔒 | `sii carpeta regular` (#109) / `sii carpeta instituciones` (#110) | `carpeta_regular` / `carpeta_instituciones` | **BLOCKED on a www2 app-session layer (found 2026-09-11, #110).** `www2.sii.cl/app/*` and its `cte-api-carpetatributaria` are authorized by a www2 APP SESSION minted by an OAuth2 code flow (`/app/session/login` → `/oauthsii-v1/` — a full Clave + reCAPTCHA Enterprise login page — → `/app/session/create`), NOT by the classic `.sii.cl` cookies the cookies-only login holds (which do reach www1/www3/www4/loa). With those cookies every `cte-api` call and `/app/session/status` answer a bare 401; the `legacy/bridge` entry points don't mint it either. Minting it is an auth-posture decision → **needs an ADR before code** (the user typing into SII's real oauth page + cookies-only capture fits ADR-006). **`instituciones` (#110) is BUILT + unit-tested, not live-validated:** `sii carpeta instituciones` / `carpeta_instituciones` (`readOnly`) read `/app/session/status` (the SPA's own session read; its `userId` keys every API path verbatim) then `GET …/instituciones` → curated `[{codigo, descripcion, abreviacion}]` (zod bare array, alias-tolerant, no hardcoded catalog); `resolveInstitucion` gates `--institucion` against the live list for #109. Without the app session the task fails with an actionable message. Session-keyed. Wire contract + the full probe table: `sii-contract/carpeta-tributaria.md`. `…/obtenerValorParametro` is NOT the PDF (#111). Milestone 0.9.1. |
...
| Carpeta tributaria | `carpeta` | instituciones (built, blocked live — #110) | regular (PDF, #109) — both gated on the www2 app-session ADR | SPA-JSON `www2` cte-api (`carpeta-tributaria-regular.ts`) · session-keyed (the www2 app session is the principal; path keyed by its `userId`) |
