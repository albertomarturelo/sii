...
| ✅ | `sii f29 pdf <periodo>` (#80) | `f29_pdf` | **Built + live-validated (#80, 2026-08-31) — the FIRST document-download surface (ADR-022).** Descarga el PDF de la declaración F29 presentada: `--tipo compacto` (el formulario como lo imprime el SII — y **el comprobante de pago** cuando el período fue pagado: su timbre dice "RECIBIDA Y PAGADA POR INTERNET", con banco/medio de pago/fecha), `solemne` (Certificado de Declaración) o `ambos`. `--out <dir>` / `directorio` eligen destino (default `~/.sii/documentos/f29`); `--folio` apunta a una declaración concreta. **El hallazgo clave:** el `codInt` que exige el servlet ES el campo `codigo` que la Fase 1 ya recibía (`getDeclaracionConEstados`) — así que NO hace falta GWT-RPC ni warm-up de SPA, al contrario que el `formCompleto` de Fase 2. Nuevos seams: `PortalSession.requestBinary` (cuerpo sin decodificar) + `FileSink` (escritura). **La task devuelve solo un descriptor (ruta/tamaño), nunca los bytes** — el documento es PII densa y no entra al contexto del LLM (ADR-006). Éxito por content-type + magic, nunca por status (SII responde 200 también en error). Contrato `sii-contract/f29-pdf.md`. |
