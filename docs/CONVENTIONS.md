...
- **Authenticated GWT-RPC read facades go through `PortalSession.requestText` (a cold replay),
  ...
  `LOGIN_HOST` bounce ⇒ `SessionExpiredError`, changed shape ⇒ "scraper roto". Body-RUT (validate
  `--rut` vs the operable set, like RCV). PII-dense (own + functionary + third-party) ⇒ NO `raw`,
  tight allowlist. (ADR-020)
