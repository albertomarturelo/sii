...
//
// EMPRESA-KEYED: `--empresa` se valida contra la lista viva del portal (`sii dte empresas`),
...
function printAvisos(avisos: readonly DteSelectAviso[]): void {
...
export function registerDte(program: Command, runtime: Runtime): void {
  const dte = program
    .command('dte')
    .description(
      'Documentos Tributarios Electrónicos (DTE). `authorized` es PÚBLICO (sin login); el resto ' +
        'opera el Portal MIPYME, empresa-keyed (--empresa validado contra el portal). SOLO ' +
        'borradores y lectura — nunca emite ni firma (ADR-023).',
    );
  ...
  dte
    .command('empresas')
    .description(
      'Empresa-keyed. Empresas para las que estás autorizado a facturar en el Portal MIPYME.',
    )
    .option('--tipo <n>', 'Tipo de DTE (33 factura, 34 exenta).', parseTipo)
    .action(async (opts: { tipo?: number }) => {
      const res = await dteEmpresas(runtime, tipoOpt(opts.tipo));
      emit(res, () => {
  ...
  borrador
    .command('list')
    .description(
      'Empresa-keyed (--empresa validado contra el Portal MIPYME). Borradores guardados de una empresa (de TODOS los tipos de DTE).',
    )
    ...
    .action(async (opts: { empresa: string; tipo?: number; soloTipo?: number }) => {
      const listed = await dteBorradorList(runtime, {
  ...
  borrador
    .command('save')
    .description(
      'Empresa-keyed (--empresa validado contra el Portal MIPYME). Crea (o actualiza con --borrador) un borrador desde un JSON. No emite nada.',
    )
    ...
    .action(
      async (
        ...
        const res = await dteBorradorSave(runtime, toArgs(readJson(json), opts));
        emit(res, () => {
          out(
  ...
  borrador
    .command('delete')
    .description(
      'Empresa-keyed (--empresa validado contra el Portal MIPYME). Elimina un borrador. Irreversible: exige --confirm con el mismo id.',
    )
    ...
    .action(async (id: string, opts: { empresa: string; tipo?: number; confirm?: string }) => {
      ...
      const res = await dteBorradorDelete(runtime, {
        empresa: opts.empresa,
        borradorId: id,
        ...tipoOpt(opts.tipo),
      });
  ...
  dte
    ...
    .action(
      async (opts: {
        ...
        const res = await dteEmitidos(runtime, {
          empresa: opts.empresa,
  ...
  dte
    .command('pdf')
    .description(
      'Empresa-keyed (--empresa validado contra el Portal MIPYME). Descarga el PDF de un documento ya EMITIDO, por folio.',
    )
    .argument('<folio>', 'Folio del documento emitido.')
    .requiredOption('--empresa <rut>', 'RUT de la empresa emisora.')
    .option('--out <dir>', 'Carpeta destino.', join(DOCUMENTOS_DIR, 'dte'))
    .action(async (folio: string, opts: { empresa: string; out: string }) => {
      ...
      const res = await dtePdf(runtime, {
        empresa: opts.empresa,
        folio: n,
        directorio: opts.out,
      });
  ...
  dte
    .command('preview')
    .description(
      'Empresa-keyed (--empresa validado contra el Portal MIPYME). Descarga el PDF de vista previa ("Validar y visualizar"). NO emite el DTE.',
    )
    .argument('<json>', 'Ruta al JSON del documento, o "-" para leer STDIN.')
    .option('--empresa <rut>', 'RUT emisor (sobrescribe el del JSON).')
    .option('--ciudad <ciudad>', 'Ciudad del emisor (el SII la exige y no la precarga).')
    ...
    .action(
      async (
        ...
        const res = await dtePreviewPdf(runtime, {
